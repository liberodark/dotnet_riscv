...
for f in artifacts/obj/*/Release/blob-feed/assets/dotnet-sdk-*.tar.gz; do
  [ -f "$f" ] || continue
  case "$f" in *"${TARGET_NAME}-riscv64.tar.gz") continue;; esac
  b=$(basename "$f"); ver=${b#dotnet-sdk-}; ver=${ver%%-linux*}
  mv "$f" "$(dirname "$f")/dotnet-sdk-${ver}-${TARGET_NAME}-riscv64.tar.gz"
done
